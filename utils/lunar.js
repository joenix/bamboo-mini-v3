...
// 二十节气相关系数
const termInfo = new Array(0, 21208, 42467, 63836, 85337, 107014, 128867, 150921, 173149, 195551, 218072, 240693, 263343, 285989, 308563, 331033, 353350, 375494, 397447, 419210, 440795, 462224, 483532, 504758);
...
function getLunar24Days(year, month, day) {
  if (_checkYear(year) && _checkMonth(month) && _checkDay(day)) {
    ...
    for (let i = 1; i <= 24; i++) {
      ...
      if (getSolarDayNumber(newDate.getFullYear(), newDate.getMonth() + 1, newDate.getDate()) == getSolarDayNumber(year, month, day)) {
        str = solarTerm[i];
        break;
      }
    }
