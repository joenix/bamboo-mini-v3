class Draw {
  ...
  drawText(text, box, style) {
    ...
    ctx.font = `${fontSize}px normal ${fontFamily}`
    ...
    const drawTextUnderLine = (text, x, y) => {
      const testWidth = ctx.measureText(text).width
      const offset = fontSize + 2
      // 绘制下划线
      ctx.beginPath();
      ctx.moveTo(x, y + offset);
      ctx.lineTo(x + testWidth, y + offset);
      ctx.strokeStyle = textUnderlineColor;
      ctx.lineWidth = textUnderlineWidth;
      ctx.stroke();
    }
    ...
    if (textWidth <= w) {
      ...
      drawTextUnderLine(text, x, y + inlinePaddingTop)
      return
    }
    ...
    for (const ch of chars) {
      ...
      if (testWidth > w) {
        ...
        drawTextUnderLine(line, x, y + inlinePaddingTop)
        y += lineHeight
        line = ch
